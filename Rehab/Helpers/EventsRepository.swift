...
//      let weekFromNow = Date().advanced(by: TimeInterval.week)
      ...
      let monthFromNow = Date().advanced(by: TimeInterval.month)
      
//      let predicate = self.eventStore.predicateForEvents(withStart: Date(), end: weekFromNow, calendars: Array(self.selectedCalendars ?? []))
      
      let predicate = self.eventStore.predicateForEvents(withStart: Date(), end: monthFromNow, calendars: Array(self.selectedCalendars ?? []))
